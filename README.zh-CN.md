...
查看从杂乱记录到运营决策的完整过程：[端到端工作示例](examples/end-to-end-operating-loop.zh-CN.md)。
...
完整的决策问题、常见误区和输出检查见[运营原则](skills/community-ops-router/references/operating-principles.zh-CN.md)。
...
把 `skills/` 下需要的文件夹复制到 Agent 使用的 Skill 目录。Codex 的全局目录是 `~/.codex/skills/`，项目目录是 `.agents/skills/`。
...
全部场景见[中文示例目录](examples/README.zh-CN.md)，每个示例都可以切换到英文版。[端到端工作示例](examples/end-to-end-operating-loop.zh-CN.md)展示 Router 如何协调多个 Skill；[SOP 与 Agent 交接示例](examples/sop-agent-handoff.zh-CN.md)展示一个重复动作如何从证据走到验证完成。
...
任何 Skill 都不能虚构缺失的指标、政策、审批、收入、留存或增长结果。

## 仓库结构

```text
skills/                 Agent Skills 和参考资料
examples/               面向新手的双语场景示例
evals/                  干净上下文中的路由和质量用例
scripts/validate.rb     本地结构、语言和隐私校验
...
贡献方式见 [CONTRIBUTING.md](CONTRIBUTING.md)。新增内容应提供独立的触发条件、运营判断、输出或证据规则。特定平台的配置应放在示例或适配层，不应写入通用核心。
